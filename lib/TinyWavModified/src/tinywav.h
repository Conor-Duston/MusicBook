...
typedef struct TinyWav {
  ...
  int32_t numFramesInHeader; ///< number of samples per channel declared in wav
                             ///< header (only populated when reading)
  uint32_t totalFramesReadWritten; ///< total numSamples per channel which have
 ...
 * @param data  A pointer to the data structure to read to. This data is
 * expected to have the correct memory layout to match the specifications given
 * in tinywav_open_read().
 * Please note that PCM data is output from the function.
 * @param buffer A pointer to an intermediate data structure for raw file data.
 ...
 * @note Samples are always expected in float32 format, regardless of file
 * sample format
 *
 * @param tw   The TinyWav structure which has already been prepared.
...
/** Returns true if the Tinywav struct is available to read or write. False
 * otherwise. */
